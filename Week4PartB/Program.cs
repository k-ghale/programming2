...
public class Program
{
    ...
    public static void WriteJson(List<Atom> atoms, string filename){
            string jsonString = JsonSerializer.Serialize(atoms, new JsonSerializerOptions { WriteIndented = true });
            File.WriteAllText(filename, jsonString);
    }

    public static List<Atom> ReadJson(string filename){
        string jsonString = File.ReadAllText(filename);
        return JsonSerializer.Deserialize<List<Atom>>(jsonString); 
    }
    ...
    public static List<Atom> ReadXml(string filename)
    {
        XmlSerializer serializer = new XmlSerializer(typeof(List<Atom>));
        using (StreamReader reader = new StreamReader(filename))
        {
            return (List<Atom>)serializer.Deserialize(reader);
        }
    }
    ...
    public static void Main(string[] args){
        ...
        //Serialize in XML and save in a suitable file.
         WriteXml(new List<Atom> { elements[0] }, "atom.xml");
        Console.WriteLine("\n Serialized first atom to XML.");

        // Deserialize and Read the XML file and display the item
        List<Atom> xmlReadAtom = ReadXml("atom.xml");
        Console.WriteLine("\n Deserialized XML atom:");
        xmlReadAtom.ForEach(Console.WriteLine);
        
        //  Serialize all elements using XML
        WriteXml(elements, "atoms.xml");
        Console.WriteLine("\n Serialized all atoms to XML.");
        
        // Serialize first element using Json
        WriteJson(new List<Atom>{elements[0]}, "atoms.json");
        Console.WriteLine("\n Serialized first atom to Json File");
        
        //Read the above file and display the output
        List<Atom> ReadJsonAtom = ReadJson("atoms.json");
        Console.WriteLine("\n Deserialized JSON atom:");
        ReadJsonAtom.ForEach(Console.WriteLine);
        
        //Serialize all atoms to a json file
        WriteJson(elements,"fullAtoms.json");
        Console.WriteLine("\n Serialized all atoms to a Json File: fullAtoms.json");
         
        //Read the above file and display the output
        List<Atom> ReadJsonAtomFull = ReadJson("fullAtoms.json");
        Console.WriteLine("\n Deserialized JSON Full atoms:");
        ReadJsonAtomFull.ForEach(Console.WriteLine);
